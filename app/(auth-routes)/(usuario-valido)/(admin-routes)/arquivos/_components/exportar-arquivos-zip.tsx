...
export default function ExportarArquivosZip({ filtros }: { filtros?: { tipos?: string, busca?: string } }) {
    ...
    const handleExportar = async () => {
        startTransition(async () => {
            ...
            try {
                ...
                const inicioResponse = await fetch(`/api/cadastro/exportar-arquivos-zip/iniciar${query ? `?${query}` : ''}`, { method: 'POST' });
    ...
    return (
        <Button variant="outline" onClick={handleExportar} disabled={isPending}>
            {isPending ? <Loader2 className='w-4 h-4 animate-spin' /> : <FileArchive className='w-4 h-4' />}
