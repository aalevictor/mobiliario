...
export async function PATCH(
    ...
    const liberado = await db.avaliacao_Licitadora.update({
        ...
        data: { liberadoAval: false }
    });
    if (!liberado) return NextResponse.json({ error: "Erro ao revogar liberação" }, { status: 500 });
    return NextResponse.json({ message: "Liberação revogada com sucesso" }, { status: 200 });
}