...
export async function DELETE(
    ...
    try {
        ...
        if (new Date(arquivo.criadoEm || 0) < dataAberturaRecurso || new Date(arquivo.criadoEm || 0) > dataLimiteRecurso) {
            return NextResponse.json({ error: "Não é possível remover documentos fora do período de recurso." }, { status: 400 });
        }
...
export async function GET(
    ...
    try {
        const session = await auth();
        if (!session) {
            console.log('GET /api/cadastro/[id]/recursos/[arquivoId] - Sem sessão');
            return NextResponse.json({ error: "Não autorizado" }, { status: 401 });
        }
        const { id, arquivoId } = await context.params;
        const cadastroId = parseInt(id);
        console.log(`GET /api/cadastro/${id}/recursos/${arquivoId} - Usuário: ${session.user.id}`);
