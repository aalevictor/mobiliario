...
export async function POST(
    ...
    if (!podeEnviarRecurso) return NextResponse.json({ error: "Não é possível atualizar os dados do cadastro fora do período de recurso." }, { status: 400 });
    const { searchParams } = new URL(request.url);
    const email = searchParams.get('email') || '';
    const eEmail = email === "true";
    try {
        ...
        for (const arquivo of arquivos) {
            ...
            const filename = `RECURSO-${eEmail ? "EMAIL-" : ""}${timestamp}-${arquivo.name.normalize('NFD').replaceAll(" ", "_").replace(/[\u0300-\u036f]/g, '').replace(/[^a-zA-Z0-9._]/g, "")}`;
