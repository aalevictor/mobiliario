...
export async function POST(
    ...
    if (!podeEnviarRecurso && !validaPermissao) return NextResponse.json({ error: "Não é possível atualizar os dados do cadastro fora do período de recurso." }, { status: 400 });
    try {
        ...
        for (const arquivo of arquivos) {
            ...
            const filename = `RECURSO-${validaPermissao ? "EMAIL-" : ""}${timestamp}-${arquivo.name.normalize('NFD').replaceAll(" ", "_").replace(/[\u0300-\u036f]/g, '').replace(/[^a-zA-Z0-9._]/g, "")}`;
